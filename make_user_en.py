...
def admin_login():
    try:
        ...
        logging.debug("✅ Admin cookie received.")
        return True
    except Exception as e:
        logging.critical(f"❌ Error during admin login: {e}")
        return False
...
def activate_extensions(user_id):
    for ext in ["lnurlp", "lndhub", "nwcprovider"]:
        ...
        try:
            ...
            logging.error(f"❌ Error activating {ext}: {e}")
...
def configure_lnaddress_public(username, pubkey):
    ...
    try:
        ...
        logging.error(f"❌ Error with LNURL for {username}: {e}")
...
def verify_nwc_pairing(secret, expected_pubkey=None):
    ...
    try:
        ...
        logging.debug(f"✅ NWC pairing link received: {pairing_link}")
    
        # Pubkey extrahieren
        actual_pubkey = pairing_link.split("://")[1].split("?")[0]
        logging.debug(f"🔍 Actual pubkey in link: {actual_pubkey}")
    
        if expected_pubkey:
            match = "✅" if actual_pubkey == expected_pubkey else "❌"
            logging.debug(f"{match} Expecteder Pubkey: {expected_pubkey}")
            if match == "❌":
                logging.warning(f"❌ Pubkey does not match! Expected: {expected_pubkey}, Erhalten: {actual_pubkey}")
    
        return actual_pubkey
    except Exception as e:
        logging.error(f"❌ Error retrieving /pairing/{secret}: {e}")
        return None
...
def create_user(username, mnemonic_words=None, nostr_pubkey_hex=None):
        ...
        if nostr_pubkey_hex:
            ...
            logging.debug(f"🔐 Using external Nostr public key for {username}: {nostr_pubkey_hex}")
        else:
            keys = generate_nostr_keys(mnemonic_words)
            logging.debug(f"🔐 Generated Nostr keys for {username}:\n{json.dumps(keys, indent=2)}")
        ...
        try:
            ...
            logging.debug(f"🧠 Expected user pubkey_hex: {keys['pubkey_hex']}")
            logging.debug(f"📅 created_at: {created_at}")
            logging.debug("=" * 80 + "\n")
        
            nwc_url, actual_nwc_pubkey, secret = create_nwc_key(
				username=username,
				wallet_id=wallet_id,
				created_at=created_at,
            ...
            if actual_nwc_pubkey:
                if actual_nwc_pubkey != keys["pubkey_hex"]:
                    logging.warning("⚠️ Pubkey differs from user pubkey!")
                    logging.warning(f"    Expected: {keys['pubkey_hex']}")
                    logging.warning(f"    Actual: {actual_nwc_pubkey}")
...
if __name__ == "__main__":
    ...
    print("  2 = 3 Generate random usernames")
    ...
    if mode == "2":
        ...
        csv_file = input(f"📂 Enter CSV filename (e.g.: ").strip()
        csv_path = Path(csv_file)
        if not csv_path.exists():
            logging.critical(f"❌ CSV file not found: {csv_file}")
            exit(1)
        ...
        logging.info(f"📄 Users loaded from file.")

    else:
        usernames = input("📝 Please enter usernames separated by spaces: ").split()
    ...
    with open(CSV_ACCOUNTS, "a", newline="") as fa, \
        ...
        for name in usernames:
            if name in existing:
                logging.info(f"⏩ User skipped (already exists): {name}")
                continue
